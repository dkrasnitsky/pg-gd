...
- Windows 10/11 x64.
- Google-аккаунт с доступом **на редактирование** ко всем таблицам, которые использует нужный вам инструмент (см. `docs/features/*.md` и `docs/onboarding-google-access.md` — там перечислены конкретные таблицы и листы для каждой фичи).
- OAuth-клиент для Google Sheets API уже встроен в сборку — при первом обращении к Sheets откроется браузер для входа в **свой собственный** Google-аккаунт — ничего выбирать/загружать вручную не нужно. Токен сохраняется локально через Electron `safeStorage`, ни сам OAuth-клиент (`electron/google-oauth-default.json`), ни токен никогда не попадают в git (в `.gitignore`), только в сам установщик.
